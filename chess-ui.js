(()=>{
'use strict';
const $=id=>document.getElementById(id),h=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])),key='abstract-autochess-run-v1';
...
function save(){if(game.phase!=='prep'&&game.phase!=='done')return;try{game.assert();localStorage.setItem(key,JSON.stringify({state:game.state(),stageChoice:$('stage-select').value}));}catch{status('本次游戏可继续；浏览器暂时无法保存进度。');}}
function syncMusic(){music.setMuted(muted);music.setPaused(document.hidden||!focused||paused);const song=game.phase==='battle'?(game.round>=10||game.players.filter(p=>p.alive).length<=3?'late':'battle'):game.phase==='done'&&game.result.rank<=3?'victory':'shop';music.play(song);$('track-title').textContent=ModeMusic.compile('chess',song).title;}
...
async function chooseStage(){const serial=++stageTicket,choice=$('stage-select').value,pool=STAGES.filter(s=>s.id!==stage?.id);const s=STAGES.find(x=>x.id===choice)||(pool.length?pool:STAGES)[Math.floor(Math.random()*(pool.length||STAGES.length))];try{const im=await art.image(s.file);if(serial!==stageTicket)return;stage=s;renderer.background=im;}catch{if(serial===stageTicket)status('场景载入失败，可换一张地图重试。');}}
for(const s of STAGES){const option=document.createElement('option');option.value=s.id;option.textContent=s.name;$('stage-select').append(option);}$('stage-select').onchange=()=>{chooseStage();save();};
function inspect(c,piece=null){inspected=c;$('inspect-name').textContent=c.name+(piece?' '+'★'.repeat(piece.star):'');$('inspect-cost').textContent=c.cost+' 金 · '+(piece?piece.star+' 星':'一星');$('inspect-traits').textContent=[...c.bonds,'基础分工：'+c.role].join(' · ');const mult=CHESS_RULES.stars[(piece?.star||1)-1];$('inspect-stats').innerHTML=[['生命',Math.round(c.hp*mult)],['攻击',Math.round(c.attack*mult)],['普攻','接触']].map(([n,v])=>`<span>${n}<b>${v}</b></span>`).join('');$('inspect-skill').textContent=c.skill.name;$('inspect-desc').textContent=c.skill.desc;const held=game.players.filter(p=>p.id!==0).reduce((n,p)=>n+game.all(p).filter(u=>u.id===c.id).reduce((a,u)=>a+3**(u.star-1),0),0);$('inspect-pool').textContent='池中剩余 '+game.pool[c.id]+' 张 · 其他棋手持有 '+held+' 张。商店展示的卡已预留，不计入池中。';queuePreview($('inspect-art'),c,false,true);updateSelection();}
...
const placement=new ChessPlacement({
 board:$('deploy-grid'),bench:$('bench'),shop:$('shop'),arena:$('chess-board'),ghost:$('drag-piece'),canMove:()=>game.phase==='prep'&&!busy&&!$('new-dialog').open&&!$('help').open&&!$('bond-help').open,
...
function newDialog(){placement.cancel();closeInfo();closeBondInfo();$('new-dialog').showModal();if(game.phase==='battle')setPaused(true);}$('new-run').onclick=newDialog;$('new-run-result').onclick=newDialog;$('new-cancel').onclick=()=>$('new-dialog').close();$('new-confirm').onclick=()=>{$('new-dialog').close();reset();};
$('resume').onclick=()=>{placement.cancel();const saved=loadedSave||stored();if(!saved)return;ticket++;art.cancelBattle();game=saved.t;battle=null;pair=null;renderer.battle=null;busy=false;paused=false;selected=null;casting=0;seenCasts.clear();$('stage-select').value=saved.stageChoice;$('resume').hidden=true;loadedSave=null;refresh();chooseStage();if(game.phase==='done'){$('result-title').textContent='上次赛事 · 第 '+game.result.rank+' 名';$('result-income').textContent='完成于第 '+game.round+' 轮。';$('next-round').hidden=true;$('new-run-result').hidden=false;}status('已恢复保存的赛事。');};
...
function frame(now){
 ...
 try{
  ...
  if(battle&&game.phase==='battle'&&!paused&&!busy&&!casting){
  ...
  playImpacts();renderer.draw(visualTime,battle?.done?1:accumulator/.05);
...
loadedSave=stored();$('resume').hidden=!loadedSave;if(loadedSave){game=loadedSave.t;$('stage-select').value=loadedSave.stageChoice;$('resume').hidden=true;loadedSave=null;}refresh();chooseStage();if(game.phase==='done'){$('result-title').textContent='上次赛事 · 第 '+game.result.rank+' 名';$('result-income').textContent='完成于第 '+game.round+' 轮。';$('next-round').hidden=true;$('new-run-result').hidden=false;}else status('先购买棋子，再点击“一键布阵”；准备好后开战。');requestAnimationFrame(frame);
})();
